...
function like(post_id) {
    ...
    xhttp.open("POST", "../api/Feed/like/" + username + "&" + post_id, true);
    xhttp.send();
}
...
function dislike(post_id) {
    ...
    if (document.getElementById("like" + post_id).style.backgroundColor == "rgb(51, 122, 183)") {
        ...
        xhttp.open("POST", "../api/Feed/dislike/" + username + "&" + post_id, true);
        xhttp.send();
    }
...
function update_number_of_likes(post_id) {
    ...
    xhttp.open("GET", "../api/Feed/" + post_id, true);
    xhttp.send();
